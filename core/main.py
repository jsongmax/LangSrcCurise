    ...
    while 1:
        ...
        try:
            ...
            try:
                ...
                Except_Log(stat=41, url=url + '|清洗数据流程获取数据失败|', error=str(e))
                return
        ...
        try:
            ...
            try:
                ...
                # 该网站使用的web容器
                Show_status = '404'
                Except_Log(stat=12, url=url + '|清洗数据流程获取数据失败|', error=str(e))
...
def Sub_Crawl(pax,Sub_Domains):
    ...
    for i in range(pool_count):
        p.apply_async(Run_Crawl,args=(Sub_Domains,))
    p.close()
    p.join()
